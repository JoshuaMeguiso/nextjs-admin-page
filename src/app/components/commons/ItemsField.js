  ...
  return (
    <div>
            ...
            onChange={(index) => {
              ...
              if (stock) {
                ...
                setItem((prevState) => {
                  return {
                    ...prevState,
                    stock: options.stocks?.[index] || null,
                    price,
                  };
                });
              }
              ...
              if (stock?._id) {
                const date = !isEmpty(state.invoice_date)
                  ? state.invoice_date
                  : state.date;

                axios
                  .post(`/api/products/${stock._id}/current-balance`, {
                    branch: state.branch,
                    date,
                  })
                  .then((response) => {
                  ...
                  .catch((err) => {
                    return message.error(
                      `There was an error retrieving balance for ${stock?.name}`
                    );
                  });
              }
      ...
      <Row>
        <Col
          span={12}
          className="is-flex flex-direction-row align-items-center"
        >
          Balance:{" "}
          <Input
